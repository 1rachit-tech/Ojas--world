...
class MessagesScreen extends StatefulWidget {
  const MessagesScreen({super.key, this.showAppBar = true});
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search name or OJAS ID',
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          subtitle: Text(
            profile.ojasId.isEmpty ? 'OJAS creator' : '@${profile.ojasId}',
            style: const TextStyle(color: Color(0xFF6B7280)),
          ),
...
class _ConversationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final profile = conversation.profileFor(otherUid);

    final displayName = _string(profile['displayName'], fallback: 'OJAS User');

    final ojasId = _string(profile['ojasId']);

    final unread = conversation.unreadCountFor(currentUid);

    final time = _formatTimestamp(conversation.lastMessageAt);
