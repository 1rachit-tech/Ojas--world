    ...
    final confirmed = await showModalBottomSheet<bool>(
      ...
      builder: (context) => SafeArea(
        child: ListTile(
          ...
          title: const Text('Delete message', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w700)),
            ...
            Expanded(
              child: StreamBuilder<OjasConversation>(
                ...
                builder: (context, snapshot) {
                  ...
                  final online = conversation?.isOnline(widget.otherUser.uid) ?? false;
                  final lastActiveAt = conversation?.lastActiveAtFor(widget.otherUser.uid);
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(widget.otherUser.displayName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF111827))),
                      Text(
                        typing ? 'typing…' : online ? 'Online' : _formatLastSeen(lastActiveAt),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(
                          fontSize: 12,
                          color: typing ? const Color(0xFF2563EB) : online ? const Color(0xFF16A34A) : const Color(0xFF6B7280),
                          fontWeight: typing || online ? FontWeight.w600 : FontWeight.w400,
                        ),
                      ),
                    ],
                  );
      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
              ...
              child: StreamBuilder<OjasConversation>(
                ...
                builder: (context, conversationSnapshot) {
                  ...
                  return StreamBuilder<List<OjasMessage>>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasError) return const _MessageLoadError();
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final createdAt = message.createdAt;
                          final seen = isMe && otherReadAt != null && createdAt != null && !createdAt.toDate().isAfter(otherReadAt.toDate());
                          final delivered = isMe && !seen && deliveredAt != null && createdAt != null && !createdAt.toDate().isAfter(deliveredAt.toDate());
            ...
            if (_replyingTo != null)
              _ReplyComposer(message: _replyingTo!, otherUserName: widget.otherUser.displayName, onCancel: _cancelReply),
            _buildInput(),
          ],
        ),
      ),
        ...
        children: [
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFFF4F5F7), borderRadius: BorderRadius.circular(24)),
              child: TextField(
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final time = ChatRoomScreenTime.format(message.createdAt);
    return Padding(
          ...
          children: [
            GestureDetector(
              ...
              child: ConstrainedBox(
                ...
                child: Container(
                  padding: message.isImage && message.hasMedia ? const EdgeInsets.fromLTRB(5, 5, 5, 8) : const EdgeInsets.fromLTRB(14, 10, 14, 8),
                  decoration: BoxDecoration(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      if (message.hasReply) _ReplyBubble(message: message, isMe: isMe, otherUserName: otherUserName),
                      if (message.isDeleted)
                        Padding(
                          padding: const EdgeInsets.all(8),
                          child: Text('This message was deleted.', style: TextStyle(color: isMe ? Colors.white70 : const Color(0xFF6B7280), fontSize: 15, fontStyle: FontStyle.italic)),
                        )
                      else ...[
                        if (message.isImage && message.hasMedia) _ChatImage(imageUrl: message.mediaUrl!, aspectRatio: message.mediaAspectRatio),
                        if (message.text.trim().isNotEmpty)
                          Padding(
                            padding: EdgeInsets.fromLTRB(message.isImage ? 8 : 0, message.isImage ? 8 : 0, message.isImage ? 8 : 0, 0),
                            child: Text(message.text, style: TextStyle(color: isMe ? Colors.white : const Color(0xFF111827), fontSize: 15, height: 1.35)),
                          ),
                      ],
                      if (time.isNotEmpty || sending)
                        Align(
                          ...
                          child: Row(
                            ...
                            children: [
                              if (time.isNotEmpty)
                                Text(time, style: TextStyle(fontSize: 10, color: isMe ? Colors.white70 : const Color(0xFF9CA3AF))),
                              if (isMe) ...[
                                if (time.isNotEmpty) const SizedBox(width: 4),
                                if (sending)
                                  const SizedBox(width: 13, height: 13, child: CircularProgressIndicator(strokeWidth: 1.7, color: Colors.white70))
                                else
                                  Icon(
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(14),
                          border: Border.all(color: mine ? const Color(0xFF9CA3AF) : const Color(0xFFE5E7EB)),
                        ),
...
class _ReplyBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: isMe ? Colors.white.withValues(alpha: 0.12) : const Color(0xFFF5F6F8), borderRadius: BorderRadius.circular(10)),
      child: Row(children: [
        Container(width: 3, height: 32, decoration: BoxDecoration(color: isMe ? Colors.white70 : const Color(0xFF6B7280), borderRadius: BorderRadius.circular(8))),
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(message.replyToType == 'image' ? '📷 Photo' : replyText, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, color: isMe ? Colors.white70 : const Color(0xFF6B7280))),
          ]),
        ),
      ]),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(width: 3, height: 38, decoration: BoxDecoration(color: const Color(0xFF111827), borderRadius: BorderRadius.circular(8))),
...
class _ChatImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => _FullScreenImage(imageUrl: imageUrl))),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(14),
        child: AspectRatio(
          ...
          child: CachedNetworkImage(
            ...
            fit: BoxFit.cover,
            memCacheWidth: 1200,
            placeholder: (context, url) => Container(color: const Color(0xFFF1F3F5), child: const Center(child: CircularProgressIndicator(strokeWidth: 2))),
            errorWidget: (context, url, error) => Container(color: const Color(0xFFF1F3F5), child: const Center(child: Icon(Icons.broken_image_outlined, size: 32, color: Color(0xFF9CA3AF)))),
          ),
        ),
      ),
    );
...
class _FullScreenImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(children: [
        Center(child: InteractiveViewer(minScale: 0.8, maxScale: 4, child: CachedNetworkImage(imageUrl: imageUrl, fit: BoxFit.contain, placeholder: (context, url) => const CircularProgressIndicator(color: Colors.white), errorWidget: (context, url, error) => const Icon(Icons.broken_image_outlined, color: Colors.white, size: 48)))),
        SafeArea(child: Padding(padding: const EdgeInsets.all(12), child: IconButton(onPressed: () => Navigator.pop(context), style: IconButton.styleFrom(backgroundColor: Colors.black54, foregroundColor: Colors.white), icon: const Icon(Icons.close_rounded)))),
      ]),
    );
  }
}
