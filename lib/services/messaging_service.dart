...
class MessagingService extends WidgetsBindingObserver {
  ...
  Stream<List<OjasMessage>> watchMessages(String conversationId) {
    return messageCollection(conversationId)
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map(OjasMessage.fromFirestore).toList());
  }
  ...
  Future<OjasProfile> _getCurrentProfile(String uid) async {
    ...
    if (!snapshot.exists || snapshot.data() == null) {
      throw const MessagingException('Your profile is not ready yet.');
    }
  ...
  void _guardLocalSend(String text) {
    ...
    if (_repeatCount >= 8 &&
        now.difference(_repeatWindowStarted) < const Duration(minutes: 1)) {
      throw const MessagingException(
