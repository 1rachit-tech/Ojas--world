...
_SAFE_AUDIO_PATH = re.compile(
    r"^creation-audio/[A-Za-z0-9_-]{1,128}/[A-Za-z0-9_-]{1,128}/[^/]{1,256}$"
)
...
def creation_playback_urls(req: func.HttpRequest) -> func.HttpResponse:
    ...
    for raw_id in raw_ids[:10]:
        ...
        creator_id = data.get("creatorId") if isinstance(data.get("creatorId"), str) else ""
        if not creator_id:
            continue
        visibility = str(data.get("visibility", "public")).strip().lower()
        deleted = data.get("deletedAt") is not None or str(data.get("moderationStatus", "")).lower() == "deleted"
        if deleted or (visibility != "public" and creator_id != uid):
            continue
        if str(data.get("mediaProvider", "")).lower() != "azure":
            continue
        if str(data.get("mediaProcessingStatus", "")).lower() not in {"ready", "published"}:
            continue

        primary_path = str(data.get("hlsStoragePath") or data.get("processedVideoStoragePath") or "").strip()
        thumbnail_path = str(data.get("thumbnailStoragePath") or "").strip()
