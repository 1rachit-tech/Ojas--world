...
(ROOT / 'lib/services/engagement_service.dart').write_text('''import 'package:cloud_firestore/cloud_firestore.dart';
...
class EngagementService {
  ...
  Future<void> syncInteraction({
    ...
    try {
      ...
      // Background engagement must never block the feed.
      // ignore: avoid_print
...
(ROOT / 'lib/widgets/reel_comments_bottom_sheet.dart').write_text('''import 'package:cloud_firestore/cloud_firestore.dart';
...
(ROOT / 'lib/widgets/reel_shop_products_bottom_sheet.dart').write_text('''import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
...
class ReelShopProductsBottomSheet extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context, {required List<String> shopItemIds}) {
    return showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
  ...
  @override
  State<ReelShopProductsBottomSheet> createState() => _ReelShopProductsBottomSheetState();
}

class _ReelShopProductsBottomSheetState extends State<ReelShopProductsBottomSheet> {
  ...
  Future<void> _loadItems() async {
    final ids = widget.shopItemIds.where((id) => id.trim().isNotEmpty).toList(growable: false);
    if (ids.isEmpty) {
    ...
    for (var start = 0; start < ids.length; start += 30) {
      ...
      final snapshot = await collection.where(FieldPath.documentId, whereIn: chunk).get();
      items.addAll(snapshot.docs.map(ShopItemModel.fromFirestore).where((item) => item.active));
    }
  ...
  String _price(ShopItemModel item) => '${item.currency} ${(item.priceMinor / 100).toStringAsFixed(2)}';
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: Container(
        ...
        decoration: const BoxDecoration(color: Color(0xFF13171D), borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
        child: Column(
          children: [
            const SizedBox(height: 10),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  const Icon(Icons.shopping_bag_rounded, color: Color(0xFFF5B942), size: 22),
                  const SizedBox(width: 8),
                  const Text('Shop Products', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 18)),
                  const Spacer(),
                  IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded, color: Colors.white54)),
                ],
              ),
            ...
            Expanded(
              child: _loading
                  ? const Center(child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFFF5B942)))
                  : _items.isEmpty
                      ? const Center(child: Text('Products unavailable', style: TextStyle(color: Colors.white54)))
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(color: const Color(0xFF1A2028), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white10)),
                              child: Row(
                                children: [
                                  ClipRRect(
                                    ...
                                    child: SizedBox(
                                      ...
                                      child: item.imageUrl.isEmpty
                                          ? const ColoredBox(color: Color(0xFF222831), child: Icon(Icons.image_outlined, color: Colors.white30))
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        Text(item.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                                        const SizedBox(height: 6),
                                        Text(_price(item), style: const TextStyle(color: Color(0xFFF5B942), fontWeight: FontWeight.w800)),
                                      ],
                                    ),
                                  ),
                                  const SizedBox(width: 8),
                                  IconButton.filled(
                                    ...
                                    style: IconButton.styleFrom(backgroundColor: const Color(0xFFF5B942), foregroundColor: Colors.black),
...
# Extend compatibility model with canonical commerce links.
p = ROOT / 'lib/models/ojs_video.dart'
s = p.read_text(encoding='utf-8')
old = '''    this.isVerified = false,
    this.viralScore = 0.0,
  })'''
new = '''    this.isVerified = false,
    this.viralScore = 0.0,
    this.shopItemIds = const [],
  })'''
if s.count(old) != 1:
    raise SystemExit('OjsVideo constructor anchor mismatch')
s = s.replace(old, new, 1)
old = '''  final bool isVerified;
  final double viralScore;
}'''
new = '''  final bool isVerified;
  final double viralScore;
  final List<String> shopItemIds;
}'''
if s.count(old) != 1:
    raise SystemExit('OjsVideo fields anchor mismatch')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')

# Feed-side optimistic interaction + comment sheet launch.
p = ROOT / 'lib/screens/ojs_feed_screen.dart'
s = p.read_text(encoding='utf-8')
old = "import 'package:flutter/services.dart';\n"
new = "import 'package:flutter/services.dart';\nimport 'package:firebase_auth/firebase_auth.dart';\n"
if s.count(old) != 1: raise SystemExit('Feed FirebaseAuth import mismatch')
s = s.replace(old, new, 1)
old = "import '../services/reel_feed_service.dart';\n"
new = "import '../services/reel_feed_service.dart';\nimport '../services/engagement_service.dart';\n"
if s.count(old) != 1: raise SystemExit('Feed engagement import mismatch')
s = s.replace(old, new, 1)
old = "import '../widgets/ojs_video_page.dart';\n"
new = "import '../widgets/ojs_video_page.dart';\nimport '../widgets/reel_comments_bottom_sheet.dart';\n"
if s.count(old) != 1: raise SystemExit('Feed comments import mismatch')
s = s.replace(old, new, 1)
old = '''  final ReelFeedService _reelFeedService = ReelFeedService();
  final List<ReelModel> _forYouReels = <ReelModel>[];'''
new = '''  final ReelFeedService _reelFeedService = ReelFeedService();
  final EngagementService _engagementService = EngagementService();
  final List<ReelModel> _forYouReels = <ReelModel>[];
  final Map<String, int> _likeDeltas = <String, int>{};'''
if s.count(old) != 1: raise SystemExit('Feed state anchor mismatch')
s = s.replace(old, new, 1)
old = '''      likes: reel.likes,'''
new = '''      likes: reel.likes + (_likeDeltas[reel.id] ?? 0),'''
if s.count(old) != 1: raise SystemExit('Feed like count anchor mismatch')
s = s.replace(old, new, 1)
old = '''      viralScore: reel.algorithmScore,
    );'''
new = '''      viralScore: reel.algorithmScore,
      shopItemIds: reel.shopItemIds,
    );'''
if s.count(old) != 1: raise SystemExit('Feed shop ids anchor mismatch')
s = s.replace(old, new, 1)
...
new = '''  void _toggleLikeVideo(String videoId) {
  ...
  void _toggleSaveVideo(String videoId) {'''
if s.count(old) != 1: raise SystemExit('Feed like method mismatch')
s = s.replace(old, new, 1)
...
if s.count(old) != 1: raise SystemExit('Feed save start mismatch')
s = s.replace(old, new, 1)
old = '''      } else {
        _savedVideos.add(videoId);
...
if s.count(old) != 1: raise SystemExit('Feed save end mismatch')
s = s.replace(old, new, 1)
...
'''
if s.count(old) != 1: raise SystemExit('Feed old comments overlay mismatch')
s = s.replace(old, '', 1)
old = '''            onComment: _toggleComments,
'''
new = '''            onComment: () => _openComments(video.id),
'''
if s.count(old) != 2: raise SystemExit('Feed comment callback mismatch')
s = s.replace(old, new)
p.write_text(s, encoding='utf-8')

# Existing video widget already owns the gesture/action overlay; only retarget commerce and double-tap semantics.
p = ROOT / 'lib/widgets/ojs_video_page.dart'
s = p.read_text(encoding='utf-8')
old = "import '../widgets/ojas_shop_sheet.dart'; // 🚀 Zero-Cost Affiliate E-Commerce\n"
new = "import 'reel_shop_products_bottom_sheet.dart';\n"
if s.count(old) != 1: raise SystemExit('Video shop import mismatch')
s = s.replace(old, new, 1)
old = '''    if (!widget.isLiked) {
      widget.onLike();
    }'''
new = '''    widget.onLike();'''
if s.count(old) != 1: raise SystemExit('Video double-tap mismatch')
s = s.replace(old, new, 1)
shop_block = '''                  // 🛍️ OJAS Shop Button
                  _buildActionButton(
...
'''
if s.count(shop_block) != 1: raise SystemExit('Video fake shop block mismatch')
s = s.replace(shop_block, '', 1)
old = '''                children: [
                  Row(
                    children: [
                      Text(
...
new = '''                children: [
                  ...
                  Row(
                    children: [
                      Text(
                        '@${widget.video.creator}','''
if s.count(old) != 1: raise SystemExit('Video metadata mismatch')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')

print('Engagement step 3 patch applied.')
